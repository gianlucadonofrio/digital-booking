...
const ReservePage = ({ getFavoritesLength }) => {
  ...
  const { data, loading } = useFetch(
    `http://digitalbooking-api-env.eba-v2srmm4z.us-east-1.elasticbeanstalk.com/booking/findByUser/${sessionStorage.getItem(
      "email"
    )}`
  );
  function rToStars(n) {
  ...
  const getFavorites = () => {
    if (logged) {
      axios
        .get(
          `http://digitalbooking-api-env.eba-v2srmm4z.us-east-1.elasticbeanstalk.com/users/favorites/${sessionStorage.getItem(
            "email"
          )}`
        )
        .then((res) => {
  ...
  const deleteBooking = (itemId) => {
    if (data) {
      fetch(
        `http://digitalbooking-api-env.eba-v2srmm4z.us-east-1.elasticbeanstalk.com/booking/delete/${itemId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: sessionStorage.getItem("jwt"),
