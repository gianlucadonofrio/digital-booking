...
const ReservePage = ({ getFavoritesLength }) => {
  ...
  const { data, loading } = useFetch(
    `https://api-digital-booking-pi.herokuapp.com/booking/findByUser/${sessionStorage.getItem(
      "email"
    )}`
  );
  function rToStars(n) {
  ...
  const getFavorites = () => {
    if (logged) {
      axios
        .get(
          `https://api-digital-booking-pi.herokuapp.com/users/favorites/${sessionStorage.getItem(
            "email"
          )}`
        )
        .then((res) => {
  ...
  const deleteBooking = (itemId) => {
    if (data) {
      fetch(
        `https://api-digital-booking-pi.herokuapp.com/booking/delete/${itemId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: sessionStorage.getItem("jwt"),
