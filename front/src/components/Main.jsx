...
const Main = ({ getFavoritesLength }) => {
  ...
  const { data, loading } = useFetch(
    "https://api-digital-booking-pi.herokuapp.com/products/findAll"
  );
  ...
  const fetchCategories = () => {
    axios
      .get(
        "https://api-digital-booking-pi.herokuapp.com/categories/findAll"
      )
      .then((res) => {
  ...
  const fetchCities = () => {
    axios
      .get(
        "https://api-digital-booking-pi.herokuapp.com/cities/findAll"
      )
      .then((res) => {
        setCities(res.data);
  ...
  const filtersSearchBlock = () => {
    ...
    if (datesSelected.length === 0 && city !== null) {
      fetch(
        `https://api-digital-booking-pi.herokuapp.com/products/findByCity/${city.innerText}/${province.innerText}/${country.innerText}`
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.length === 0) {
    ...
    if (datesSelected.length === 2 && city === null) {
      ...
      fetch(
        `https://api-digital-booking-pi.herokuapp.com/products/findByDate/${startDate}/${endDate}`
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.length === 0) {
    ...
    if (datesSelected.length === 2 && city !== null) {
      ...
      axios
        .get(
          `https://api-digital-booking-pi.herokuapp.com/products/findByCityAndDate/${city.innerText}/${startDate}/${endDate}`
        )
        .then((res) => res.data)
        .then((data) => {
          if (data.length === 0) {
